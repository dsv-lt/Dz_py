...
def runnet(x1, x2):
    ...
    # 3-й скрытый нейрон (новый)
    input3h = i1 * bestweight[6] + i2 * bestweight[7] + b1best
    output1h = sigm(input1h)
    output2h = sigm(input2h)
    output3h = sigm(input3h)
    # Выходной нейрон
    inputo = (
        output1h * bestweight[4]
        + output2h * bestweight[5]
        + output3h * bestweight[8]
        + b2best
    )
    outputo = sigm(inputo)
    return outputo
...
for ep in range(ept):
    ...
    for i in range(length):
        ...
        # 3-й скрытый нейрон (новый)
        input3h = i1 * weight[6] + i2 * weight[7] + b1
        output1h = sigm(input1h)
        output2h = sigm(input2h)
        output3h = sigm(input3h)
        # Выходной нейрон получает 3 входа от скрытого слоя
        inputo = (
            output1h * weight[4]
            + output2h * weight[5]
            + output3h * weight[8]
